...
- **Spell checking in the WYSIWYG editor.** Misspelled words now get a wavy underline, and right-clicking one offers up to five corrections plus **Add to Dictionary** and **Ignore**. VS Code turns off the browser's built-in spell checker for every webview, which is why MikeDown's editor never showed squiggles even when the plain text editor did — so MikeDown now ships its own hunspell dictionaries and checks entirely offline. Code blocks (including mermaid), inline code, links, and `[[wikilinks]]` are left alone, and the checker skips the word you're in the middle of typing. Choose US or UK English with `mikedown.spellCheck.language`, keep your own word list in `mikedown.spellCheck.userWords` (words you've already taught the Code Spell Checker extension are honoured too), or turn the whole thing off with `mikedown.spellCheck.enabled`. All four settings live in the new **Spelling** tab of the in-editor settings.
...
- **The custom dictionary in the Spelling tab now scales past a handful of words.** It was a flat cloud of chips in insertion order, which stopped being usable somewhere around a dozen words and was unreadable at a hundred. The list is now sorted alphabetically (case-insensitively) at display time — the saved `mikedown.spellCheck.userWords` keeps its own order, so opening the settings no longer churns your `settings.json` — and once there are eight or more words a filter box appears above it with a live count (`12 of 87`), so a filtered view never reads as "my words vanished". Past 25 words the chip cloud gives way to a columned list, one word per row, because wrapped chips of varying width defeat an alphabetical scan no matter how well sorted they are — at 100 words that's four columns and about thirteen rows in view instead of twenty rows of ragged pills. Removal is hover-revealed there rather than a hundred always-on × glyphs. Empty results say whether the dictionary is empty or the filter simply matched nothing. Words inherited from the Code Spell Checker extension (`cSpell.words`) are still merged in read-only, now as dashed chips with a tooltip explaining where to edit them. Removing a word after filtering or sorting used to delete whichever word happened to sit at that position in the underlying list — a different word than the one you clicked; removal now goes by value. Keyboard users get labelled filter and remove controls, Escape clears the filter without closing the settings, and focus lands on the neighbouring chip after a removal.
- **Spell checking now ships off by default** (`mikedown.spellCheck.enabled`). Turn it on in the editor's Settings → Spelling tab or in VS Code settings.
...
- **First line of a multi-line code block no longer looks indented** ([#2](https://github.com/mikejoseph23/mikedown/issues/2)). The inner `<code>` is inline and carried `0.4em` of horizontal padding, which on an inline element lands only at the start of the first line — nudging it ~one character right of the lines below. Its horizontal padding is now zeroed; the line-highlight background and syntax colors are unchanged.
